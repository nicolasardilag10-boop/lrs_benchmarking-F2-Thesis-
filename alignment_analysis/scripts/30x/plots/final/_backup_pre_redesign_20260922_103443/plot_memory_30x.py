...
OUTPUT_DATA = (
    ...
    / "derived"
    / "plot_data"
    / "memory_30x_plotting_values.tsv"
)
...
def load_data() -> pd.DataFrame:
    ...
    if not INPUT_TSV.is_file():
        raise FileNotFoundError(
            f"Benchmark table not found: {INPUT_TSV}"
        )
    ...
    if data.duplicated(keys).any():
        duplicates = data[
            data.duplicated(keys, keep=False)
        ][keys]
    ...
    missing_rows = expected.difference(
        observed
    ).tolist()

    if missing_rows:
        raise ValueError(
            f"Missing benchmark combinations: {missing_rows}"
        )
    ...
    if data[
        ...
        bad = data[
            data["command_ram_limit_gb"].isna()
        ][
            [
                "sample",
                "read_technology",
        ...
        raise ValueError(
            "Configured RAM limits must be present "
            "for all rows:\n"
            + bad.to_string(index=False)
        )

    # Peak RSS is currently available only for
    # minimap2 and pbmm2.
    measured = data[
        data["peak_ram_gb"].notna()
    ]
...
def prepare_plot_data(
    ...
    # Check that configured RAM does not vary
    # between samples or technologies for an aligner.
    limit_counts = (
        data.groupby("aligner")[
            "command_ram_limit_gb"
        ]
        .nunique()
    )

    if not limit_counts.reindex(
        ALIGNERS
    ).eq(1).all():

        raise ValueError(
            "Configured RAM limits vary "
            "between runs for an aligner"
        )

    # Panel A:
    # configured RAM limit.
    #
    # Technology does not change the RAM limit,
    # so each sample is shown once per aligner.
    limits = (
        data.groupby(
            [
                "sample",
...
def main() -> int:
    ...
    # Hatch explanation belongs only
    # to configured-limit panel.
    limit_handle = Patch(
        facecolor="white",
        edgecolor="#222222",
        hatch="////",
    ...
    # Center sample legend relative
    # to actual plotting area.
    axes_center_x = (
        axes[0].get_position().x0
        + axes[0].get_position().x1
    ) / 2
