#!/usr/bin/env python3
"""Plot mapped and unmapped reads (%) for minimap2, pbmm2, VACmap and VG Giraffe.

Stacked bar (mapped reads in the sample color, unmapped reads in a
lightened tint of the same color), from a truncated 90-100% baseline.

Denominator
-----------
Percentages are computed against the verified raw input read count per
sample/technology, not each aligner's own `raw_total_sequences`. minimap2,
pbmm2 and VG Giraffe keep unmapped reads in their output and agree exactly
on `raw_total_sequences`. VACmap drops unmapped reads (`reads_unmapped == 0`),
so its own count excludes them and would report 100% mapped. Using the
shared raw input adds VACmap's dropped reads back as unmapped, making it
comparable to the other aligners. Same logic as plot_raw_total_count.py.
"""
...
OUTPUT_SUMMARY = TABLE_DIR / "derived" / "plot_data" / "mapped_unmapped_reads_percent_30x.tsv"
...
def load_data() -> pd.DataFrame:
    ...
    required_columns = {"sample", "read_technology", "aligner", "raw_total_sequences", "reads_mapped", "reads_unmapped"}
    ...
    plot_data = data.loc[
        ...
        ["sample", "read_technology", "aligner", "raw_total_sequences", "reads_mapped", "reads_unmapped"],
    ].copy()

    for column in ["raw_total_sequences", "reads_mapped", "reads_unmapped"]:
        plot_data[column] = pd.to_numeric(plot_data[column], errors="raise")

    # Verified raw input per sample/technology: the raw_total_sequences that
    # all aligners retaining unmapped reads (reads_unmapped > 0) agree on.
    retaining = plot_data.loc[plot_data["reads_unmapped"] > 0]
    raw_input = retaining.groupby(["sample", "read_technology"])["raw_total_sequences"].agg(["nunique", "first"])
    if (raw_input["nunique"] != 1).any():
        raise ValueError(
            "Aligners retaining unmapped reads disagree on raw_total_sequences:\n"
            + raw_input.loc[raw_input["nunique"] != 1].to_string()
        )
    plot_data = plot_data.merge(
        raw_input["first"].rename("raw_input_reads").reset_index(), on=["sample", "read_technology"], how="left",
    )
    if plot_data["raw_input_reads"].isna().any():
        raise ValueError("No aligner retains unmapped reads for at least one sample/technology.")
    if (plot_data["reads_mapped"] > plot_data["raw_input_reads"]).any():
        raise ValueError("reads_mapped exceeds the verified raw input for at least one row.")

    # Reads the aligner dropped from its output (non-zero only for VACmap).
    plot_data["dropped_unmapped_reads"] = plot_data["raw_input_reads"] - plot_data["raw_total_sequences"]
    plot_data["mapped_reads_percent"] = 100.0 * plot_data["reads_mapped"] / plot_data["raw_input_reads"]
    plot_data["unmapped_reads_percent"] = 100.0 - plot_data["mapped_reads_percent"]
...
def main() -> int:
    ...
    for panel_index, technology in enumerate(TECHNOLOGY_ORDER):
        ...
        for sample in SAMPLE_ORDER:
            ...
            mapped_values = sample_data["mapped_reads_percent"].to_numpy(dtype=float)
            unmapped_values = sample_data["unmapped_reads_percent"].to_numpy(dtype=float)
